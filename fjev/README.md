## fjev = "formális jogi egységekre vagdosás" = "cutting into formal legal units"

This script explores the structure of hierarchical documents (e.g. law).

### getting started in 5 minutes

Usage: type `make` and a file called `out` is produced
...
_consecutive_ numberind. That means, in practice,
if we have had "Article\_1" yet, so we are inside the first article,
the coming "Article\_2" will mark the next article,
but "Article\_42" in the text meanse something else,
usually a reference to another unit. 
...
A _marktype_ is the format of the mark
which shows the numberer part and the fixed part of it separately.
...
plain words looking like a mark gets an "simple" annotation.
...
which is inside a type 0 mark (`t0`) in the end.
...
`<-up2` means that we came outer by 2 levels
 ...
 * `[[X:"word"message]]`
i.e. `word` is the word being annotated.
And `message` can be one of the following:
   * `mayberef<-nextword` = based on the next word
this is not considered as a mark (it is likely a legal reference)
   * `mt1st2x` = this is #0 item of a marktype
...
To modify `marktypes` just edit / add /remove some lines to is.

To add a new numberer is something more complicated.
